...
const megaMenus = {
    ...
    columns: [
      ...
      },
      {
        items: [
          ...
          /* Workshop removed as per requirement */
        ],
      },
    ],
  },
    ...
    columns: [
      {
        ...
        items: [
          ...
          {
            label: "Physiotherapy in Cardio-pulmonary Lab",
            href: "#",
            icon: Stethoscope,
          },
  ...
  Alumni: {
    ...
    left: [
      ...
      /* Placement removed as per requirement */
          ...
          <div className="flex justify-center gap-1">
            {navigationItems.map((item) => (
              ...
              >
                <Link
                  to={item.href}
                  className="px-3 py-3 flex items-center gap-1 text-white uppercase text-[13px] font-semibold tracking-wide hover:bg-primary/80 transition-colors whitespace-nowrap"
                >
                  {item.label}
                                ...
                                <div className="grid grid-cols-1 gap-2">
                                  {column.items.map((item: any) => {
                                    ...
                                    return (
                                      <div
                                        key={item.label}
                                        className="bg-gray-50 hover:bg-primary rounded-md p-2 transition-all duration-300 group shadow-sm border border-gray-200 hover:shadow"
                                      >
                                        <Link
                                          to={item.href}
                                        ...
                                        >
                                          ...
                                          <span className="text-[13px] font-medium leading-tight text-gray-700 group-hover:text-white">
                                            {item.label}
                                          </span>
                                        </Link>
                                      </div>
                  ...
                  {activeMenu === "Infrastructure" && (
                      <div className="mx-auto" style={{ width: "65vw" }}>
                        <div className="grid grid-cols-4 gap-4">
                          {/* Left - Physiotherapy Labs */}
                          <div className="col-span-2 space-y-3">
                            <h4 className="text-base font-bold text-primary uppercase tracking-wide text-center pb-1.5 border-b border-primary/30">
                              PHYSIOTHERAPY LABS
                            </h4>
                            <div className="grid grid-cols-2 gap-2.5">
                              {megaMenus["Infrastructure"].columns[0].items.map((item) => {
                                const Icon = item.icon;
                                return (
                                  <div
                                    key={item.label}
                                    className="bg-gray-50 hover:bg-primary rounded-md p-2 transition-all group shadow-sm border"
                                  >
                                    <Link
                                      to={item.href}
                                      className="flex flex-col items-center text-center"
                                    >
                                      <div className="p-1.5 bg-white rounded-full shadow-sm group-hover:bg-white/20 mb-0.5">
                                        <Icon className="w-5 h-5 text-primary group-hover:text-white" />
                                      </div>
                                      <span className="text-[12px] font-medium leading-tight text-gray-700 group-hover:text-white">
                                        {item.label}
                                      </span>
                                    </Link>
                                  </div>
                          ...
                          {/* Right - General Structure */}
                          <div className="col-span-2 space-y-3">
                            <h4 className="text-base font-bold text-primary uppercase tracking-wide text-center pb-1.5 border-b border-primary/30">
                              GENERAL STRUCTURE
                            </h4>
                            <div className="grid grid-cols-2 gap-2.5">
                              {megaMenus["Infrastructure"].columns[1].items.map((item) => {
                                const Icon = item.icon;
                                return (
                                  <div
                                    key={item.label}
                    ...
                    {/* STUDENT LIFE - Changed to 2x2 grid (2 rows, 2 columns) */}
                    ...
                    {/* ALUMNI - Placement removed, now 2 items in 2-column layout */}
                    {activeMenu === "Alumni" && (
                      <div className="max-w-4xl mx-auto relative">
                        <div className="grid grid-cols-2 gap-6">
                          {megaMenus["Alumni"].left.map((item: any) => {
                            const Icon = item.icon;
                            return (
                              <div
                                key={item.label}
